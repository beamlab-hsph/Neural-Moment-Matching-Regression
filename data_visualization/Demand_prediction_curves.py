...
for i, method_dir in enumerate(method_dirs):
    ...
    result_path = op.join(method_dirpath, f"n_sample:{sample_size_to_plot}")
...
fig.suptitle(f"Predictions of E[Y | do(A)] by each method (n={sample_size_to_plot})")
for ax in axs.flat:
    ax.label_outer()
